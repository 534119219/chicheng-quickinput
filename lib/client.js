...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function maskPrivateValue(value, isPrivate) {
		...
		function VaultPanel(props) {
			...
			var recordFromSuggestion = useCallback(function (item) {
				var name = item.category + " · " + (item.private === true ? "****" : String(item.value).slice(0, 24));
				setModal({
					kind: "add",
					preset: {
						name: name,
					...
					createElement("div", { className: "dsh-vault-scroll", style: { display: "flex", flexDirection: "column", gap: 6, maxHeight: 92, overflowY: "auto" } },
						suggestions.slice(0, 3).map(function (item) {
							return createElement("div", { key: item.id, style: { display: "flex", alignItems: "center", gap: 8, minWidth: 0 } },
								...
								createElement("span", { style: { flex: 1, minWidth: 0, fontSize: 11.5, color: "var(--dsw-alias-label-secondary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "ui-monospace,Menlo,Consolas,monospace" } }, maskPrivateValue(item.value, item.private)),
		...
		function RecordRow(props) {
			...
			return createElement("div", {
				...
				createElement("div", { style: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 3 } },
					...
					createElement("div", { style: { fontSize: 10.5, color: "var(--dsw-alias-label-tertiary)", opacity: 0.8 } }, t("createdAt") + " " + fmtTime(rec.createdAt))
				),
				createElement("button", {
					type: "button",
