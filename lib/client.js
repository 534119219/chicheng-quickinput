	...
	factory: (require) => {
		...
		function VaultPanel(props) {
			...
			var header = createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, padding: "12px 12px 0", flex: "none" } },
				...
				createElement("button", {
					type: "button",
					onClick: function () { setModal({ kind: "settings" }); },
					style: iconBtnStyle,
					title: t("settings"),
					"aria-label": t("settings"),
				}, createElement(IconSettingsOutline16, { size: 15 })),
				unlocked
					? createElement("button", {
						type: "button",
						onClick: lockPanel,
			...
			var suggestionBanner = suggestions.length > 0
				? createElement("div", { style: { margin: "10px 12px 0", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 12, background: "var(--dsw-alias-bg-layer-2)", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 8, flex: "none" } },
					...
					createElement("span", { style: { fontSize: 11.5, color: "var(--dsw-alias-label-tertiary)", lineHeight: 1.5 } }, t("suggestHint")),
					createElement("div", { className: "dsh-vault-scroll", style: { display: "flex", flexDirection: "column", gap: 6, maxHeight: 132, overflowY: "auto" } },
						suggestions.slice(0, 5).map(function (item) {
					...
					suggestions.length > 5
						? createElement("div", { style: { fontSize: 11, color: "var(--dsw-alias-label-tertiary)" } }, t("more", { n: suggestions.length - 5 }))
						: null
				)
				: null;
			...
			return createElement(ModalShell, { title: editing ? props.t("formEdit") : props.t("formAdd"), onClose: props.onClose },
				...
				createElement(Field, { label: props.t("formCategory") },
					createElement("input", {
						...
						placeholder: "密钥 / 服务器 / 地址 / 手机号 / 网址 / 其他",
					}),
		...
		function SettingsModal(props) {
			...
			var doUnlock = function () {
				setUnlockBusy(true);
				setUnlockErr("");
				props.onUnlockCheck(unlockPw).then(function (ok2) {
					setUnlockBusy(false);
					if (!ok2) setUnlockErr(props.t("unlockFail"));
					else setUnlockPw("");
				});
			};
			...
			var saveWebdav = function () {
				...
				setError("");
				setBusy("save");
				props.onSaveWebdav({ url: String(url).trim(), username: String(user).trim(), password: pass })
					.then(function () { setOk(props.t("webdavSaved")); })
					.catch(function (e) { setError(String((e && e.message) || "error")); })
					.finally(function () { setBusy(""); });
			};

			var doBackup = function () {
				if (!props.webdav || String(props.webdav.url).trim() === "") { setError(props.t("webdavUrlRequired")); return; }
				var filename = String(backupName).trim() || "chicheng-vault-backup.json";
				setBusy("backup");
				setOk("");
				setError("");
				props.onBackup(filename).then(function () {
			...
			var doRestore = function () {
				...
				setOk("");
				setError("");
				props.onRestore(filename).then(function () {
			...
			var doChangePw = function () {
				...
				setPwMsg("");
				props.onChangePassword(curPw, newPw).then(function () {
			...
			return createElement(ModalShell, { title: props.t("settings"), onClose: props.onClose, topAlign: true, width: 560 },
				// 自动识别
				...
				createElement("div", { style: sectionStyle },
					...
					createElement("span", { style: sectionDescStyle }, props.unlocked ? props.t("webdavDesc") : props.t("webdavLocked")),
					!props.unlocked
						? createElement("div", { style: { display: "flex", gap: 8, alignItems: "flex-end" } },
							createElement("div", { style: { flex: 1 } },
			...
			switch (modal.kind) {
				...
				case "settings":
					return createElement(SettingsModal, {
						t: m.t,
						autoDetect: runtime.status ? runtime.status.autoDetect : true,
						unlocked: m.unlocked,
						webdav: m.webdav,
						autoLockMinutes: m.autoLockMinutes,
						onClose: function () { m.setModal(null); },
						onSetAutoDetect: m.setAutoDetect,
						onSetAutoLock: m.setAutoLock,
						onUnlockCheck: m.doVerify,
						onSaveWebdav: m.saveWebdav,
						onBackup: m.doBackup,
						onRestore: m.doRestore,
						onChangePassword: m.doChangePw,
						onShowReset: function () { m.setModal({ kind: "reset" }); },
					});
				case "setupDone":
