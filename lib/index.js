...
const IP_PATTERNS = [
  ...
  // IPv6 完整段：至少 4 段 + 尾段（5~8 段），避免把 HH:MM:SS 时间当成 IPv6
  /\b(?:[0-9a-fA-F]{1,4}:){4,7}[0-9a-fA-F]{1,4}\b/g,
  // IPv6 :: 简写（多段 + ::，如 2001:db8::1）；至少 3 个冒号，时间格式不满足
  /\b(?:[0-9a-fA-F]{1,4}:){2,6}:(?:[0-9a-fA-F]{1,4})?\b/g,
  // IPv6 :: 简写（单段 + ::，如 fe80::1）
  /\b[0-9a-fA-F]{1,4}::(?:[0-9a-fA-F]{1,4})?\b/g,
  // IPv6 纯 :: 开头（如 ::1）
  /\b::[0-9a-fA-F]{1,4}\b/g,
];
