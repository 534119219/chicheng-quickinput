...
- 最新提交：`272497b`（7 个提交，全部已推送，工作区干净）
...
2. 面板内遗留**死代码**：VaultPanel 的 `setAutoDetect/setAutoLock/saveWebdav/doBackup/doRestore/doChangePw` 回调不再被引用（设置逻辑已移入 SettingsBody）——可清理
3. client.js 里若干未用 import/变量（`IconSettingsOutline16`、`useMemo` 等）——可清理
